...
- Independent nuclear family structure: Generates parent-offspring trios and sibling groups.
- Multi-site genetic architecture: Implements a designated Trait site (S0) and M additional marker loci.
- Binary phenotype labeling: Assigns 'y' based on Dominant/Recessive models with variable penetrance.
- Systematic masking: Simulates incomplete data by masking parental genotypes (linked across all loci for a specific parent).
- Wide-format export: Optimized data structure for Bayesian, HMM, and ML model ingestion.
- Reproducible seeds: Ensures consistency across ancestry, inheritance, and masking steps.
...
def main():
    ...
    print(f"Saved: {f'{base_filename}_masked_out.csv'}")
    print(f"Saved: {f'{base_filename}_unmasked_out.csv'}")
